-- =============================================================================
-- Fix: generate_practice_session dynamic query parameter binding
-- The dynamic SQL was referencing the PL/pgSQL variable `p_config` by name,
-- which causes a "column does not exist" error. It must use $1.
-- =============================================================================
...
BEGIN
    ...
    IF (p_config->>'questionCount')::integer > 0 THEN
        -- We don't need $1 here because this value is interpolated directly into the string
        -- BUT it's safer to use parameterized queries if possible. Since we already cast to integer
        -- and concatenate, it's safe from SQL injection, but using p_config in the IF block is fine.
