...
-- Apply to BOTH submit_question_attempt (practice) and
-- submit_mock_test_attempts (mock test).
-- ─────────────────────────────────────────────────────────────────────────────

-- Practice-mode RPC: add per-question time clamping + session wall-clock check
...
BEGIN
    ...
    -- Server-side wall-clock check (5 min grace) for timed sessions
    ...
    -- Clamp time_taken_seconds to session duration (prevents inflated stats)
    v_clamped_time := p_time_taken_seconds;
    IF v_session_record.duration_minutes > 0 AND v_clamped_time > (v_session_record.duration_minutes * 60) THEN
        v_clamped_time := v_session_record.duration_minutes * 60;
    END IF;
    ...
    INSERT INTO public.question_attempts (
        ...
        auth.uid(), p_question_id, v_is_correct, p_exam_type, v_clamped_time, p_session_id
    )
...
-- ─────────────────────────────────────────────────────────────────────────────
-- FIX #3: ADMIN AUTH — Explicit per-operation mutation policies
-- NOTE: The initial schema's "Allow admin full access on X" FOR ALL policies
-- already cover INSERT/UPDATE/DELETE. These per-operation policies serve as
-- defense-in-depth: if the FOR ALL policy is ever accidentally dropped,
-- these explicit policies still block non-admin mutations.
-- ─────────────────────────────────────────────────────────────────────────────
