...
import { Bookmark, Search, Folder, Filter, X } from 'lucide-react';
...
export const BookmarksPage: React.FC = () => {
  ...
  useEffect(() => {
    if (user) {
      supabase.from('bookmarks').select('question_id').eq('user_id', user.id).then(({ data }) => {
        if (data) {
          setBookmarkedIds(data.map(d => d.question_id));
        }
      });
    }
  }, [user]);
  ...
  }, [questions, bookmarkedIds, academicTree]);

  const classes = useMemo(() => Array.from(new Set(bookmarkedQuestions.map(q => q.path.className).filter(c => c !== 'Unknown'))), [bookmarkedQuestions]);
  const subjects = useMemo(() => Array.from(new Set(bookmarkedQuestions.filter(q => selectedClass === 'All' || q.path.className === selectedClass).map(q => q.path.subjectName).filter(s => s !== 'Unknown'))), [bookmarkedQuestions, selectedClass]);
  const chapters = useMemo(() => Array.from(new Set(bookmarkedQuestions.filter(q => (selectedClass === 'All' || q.path.className === selectedClass) && (selectedSubject === 'All' || q.path.subjectName === selectedSubject)).map(q => q.path.chapterName).filter(c => c !== 'Unknown'))), [bookmarkedQuestions, selectedClass, selectedSubject]);

  const displayQuestions = bookmarkedQuestions
                ...
                {groupedQuestions[groupKey].map((q, qIndex) => (
                  ...
                  >
                     <button 
                        ...
                        className="absolute top-4 right-4 text-blue-500 hover:text-blue-600 transition-colors"
      ...
      {totalPages > 1 && (
        <div className="flex items-center justify-between mt-8 p-4 bg-geist-surface-light dark:bg-geist-surface-dark border border-geist-border-light dark:border-geist-border-dark rounded-xl">
           <span className="text-xs font-medium text-geist-text-secondary-light dark:text-geist-text-secondary-dark">
             Showing {Math.min((currentPage - 1) * ITEMS_PER_PAGE + 1, displayQuestions.length)} to {Math.min(currentPage * ITEMS_PER_PAGE, displayQuestions.length)} of {displayQuestions.length} bookmarks
           </span>
