...
export const QuestionTypesManager: React.FC = () => {
  const { questionTypes, addQuestionType, deleteQuestionType, fetchCoreData, reorderQuestionTypes } = useAdminStore();
  ...
  const handleAddType = async () => {
    ...
    try {
      await addQuestionType({ id: uuidv4(), name: newTypeName });
      setNewTypeName('');
    } catch (e: any) {
      alert("Failed to add question type: " + e.message);
    }
  };
  ...
  return (
    <div className="max-w-4xl mx-auto p-4 md:p-8 font-sans bg-geist-bg-light dark:bg-geist-bg-dark h-full overflow-y-auto">
      <div className="mb-8">
        <h1 className="text-2xl font-semibold text-geist-text-primary-light dark:text-geist-text-primary-dark">Question Types Manager</h1>
        <p className="text-geist-text-secondary-light dark:text-geist-text-secondary-dark mt-1">Manage custom Question Types.</p>
      </div>
      ...
      <div className="bg-geist-surface-light dark:bg-geist-surface-dark border border-geist-border-light dark:border-geist-border-dark rounded-xl p-4">
        <h2 className="text-lg font-medium text-geist-text-primary-light dark:text-geist-text-primary-dark mb-4 flex items-center gap-2">
          <Layout className="w-5 h-5" /> Question Types
        </h2>
        <div className="flex gap-2 mb-4">
          <input 
            ...
            placeholder="E.g., Assertion, Numerical..." 
          ...
          />
          <button onClick={handleAddType} className="bg-geist-text-primary-light dark:bg-geist-text-primary-dark text-geist-bg-light dark:text-geist-bg-dark px-3 py-2 rounded-lg hover:opacity-90">
        ...
        <div className="space-y-2">
          {questionTypes.map((qt, index) => (
            <div 
              ...
              onDrop={(e) => {
                ...
                if (draggedIndex !== null && draggedIndex !== index) {
                  reorderQuestionTypes(draggedIndex, index);
                }
            ...
            >
              ...
              <button onClick={() => deleteQuestionType(qt.id)} className="text-geist-error-light dark:text-geist-error-dark hover:opacity-70 p-1">
                <Trash2 className="w-4 h-4" />
              </button>
            </div>
          ))}
          {questionTypes.length === 0 && <div className="text-sm text-geist-text-secondary-light text-center py-4">No types defined.</div>}
        </div>
      </div>
    </div>
  );
